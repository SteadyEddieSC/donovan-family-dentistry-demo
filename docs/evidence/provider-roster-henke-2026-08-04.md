# Provider roster evidence — Dr. Jordan Henke and updated provider photography

**Recorded:** 2026-08-04  
**Source:** Practice-supplied request forwarded by Dr. William Donovan

## Approved public update

The practice supplied:

- a current Dr. Jordan Henke biography;
- a Henke family photograph;
- the requested family caption **Henke Family**;
- an instruction to remove the outdated Koolkin reference; and
- `IMG_4850` as the updated photograph for Dr. William Donovan.

## Published roster

The release candidate publishes:

1. Dr. William Donovan, DMD
2. Dr. Jordan Henke, DDS

The current provider data does not publish Dr. Robert Koolkin or an unapproved associate-dentist template.

## Implemented routes

- Classic: `/about/`
- Modern: `/modern/team/`

## Image handling

- `IMG_4850` was decoded from its original HEIC file using libheif, cropped to a balanced landscape composition, and assigned to Dr. Donovan.
- The supplied Henke family photograph was cropped to a landscape composition retaining Dr. Henke, Mia, and all four children.
- Both photographs are stored locally as validated 480 × 360 WebP assets, with 360-pixel and compatibility 480-pixel derivatives generated during the build.
- The materialization step verifies the exact byte length and SHA-256 digest before writing either approved photograph.
- No third-party image host or paid image service is used.

## Remaining boundary

This evidence clears the provider-roster publication question only. It does not approve services, insurance or payment wording, urgent-care instructions, production integrations, indexing, DNS cutover, or patient-data handling.

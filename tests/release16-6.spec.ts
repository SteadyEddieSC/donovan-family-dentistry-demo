...
test('save contact is the first and visually distinct contact action', async ({ page }) => {
  await page.goto('/modern/contact/');

  const actions = page.locator('.modern-contact-stack > a');
  await expect(actions).toHaveCount(4);
  await expect(actions.nth(0)).toContainText('Save office contact');
  ...
  expect(featuredColors.background).toBe('rgb(216, 239, 123)');
...
test('New Patients exposes the same save-contact utility near the top of the journey', async ({ page }) => {
  await page.goto('/modern/new-patients/');

  const saveContact = page.getByRole('link', { name: /Save the office contact/ });
...
test('contact and New Patients action surfaces remain usable on a narrow phone', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /Save the office contact/ })).toBeVisible();
});

test('Release 16.6 action surfaces have no serious or critical automated accessibility violations', async ({ page }) => {
  for (const path of ['/modern/contact/', '/modern/new-patients/']) {
    await page.goto(path);
